...
driver.get('https://sports.news.naver.com/wfootball/schedule/index.nhn?year=2020&month=05&category=seria')
...
data=[]
for i in range(len(day)):
    data.append((dt[i],dt[i]+datetime.timedelta(hours=2),left_team[i]+" vs "+right_team[i],place[i],"Serie A",41,datetime.datetime.now(),datetime.datetime.now()))
print(data)
